...
class VisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        # if patch size is the one defined in initialization
        if npatch == N and w == h: 
    ...
    def set_patch_drop(self,drop=0.):
        self.patch_drop = drop
    ...
    def forward(self, x):
        ...
        # separate the data by patch size
        ...
        for end_idx in idx_crops:
            x_idx = self.prepare_tokens(torch.cat(x[start_idx: end_idx]))
            for blk in self.blocks:
                x_idx = blk(x_idx)
            x_idx = self.norm(x_idx)
            h_idx = x_idx[:, 0]
...
def vit_tiny(patch_size=16, in_chans=1, **kwargs):
...
def vit_small(patch_size=16, in_chans=1, **kwargs):
    model = VisionTransformer(
